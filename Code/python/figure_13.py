...
def init_constant():
    ...
    constant[Fhold] = 5.0
    constant[Ftest] = 5.0 
    constant[Fton] = 90000.0 
    constant[Ftoff] = 330000.0 
...
if __name__ == "__main__":
    ...
    # --- 1. SIMULATION 1 : Cas Standard (Effet chimique + électrique) ---
    # On s'assure que p3 est à sa valeur normale (0.01)
    print("Simulation 1 : Cas standard...")
    voi, state_std, algebrica_std = solve_model() 
    
    # --- 2. SIMULATION 2 : Cas "Électrique Pur" (Pointillés) ---
    # On va modifier temporairement p3 pour désactiver l'activation par le Calcium
    print("Simulation 2 : Sans activation enzymatique (p3 élevé)...")
    
    constant_p3 = init_constant()
    constant_p3[p3] = 0.0 
    
    # Lancement de la deuxième simulation
    r2 = ode(calcul_rate)
    r2.set_integrator('vode', method='bdf', atol=1e-06, rtol=1e-06)
    r2.set_initial_value(init_state(), voi[0])
    r2.set_f_params(constant_p3)
    
    state_p3 = np.zeros((size_state, len(voi)))
    state_p3[:, 0] = init_state()
    for i in range(1, len(voi)):
        r2.integrate(voi[i])
        state_p3[:, i] = r2.y
    algebrica_p3 = calcul_algebrica(voi, state_p3, constant_p3)
    ...
    # Liste des variables à tracer pour automatiser
    # Format : (axe_y, axe_x, data_std, data_p3, titre, label_y)
    plots = [
        (0, 0, state_std[c, idx], state_p3[c, idx], '(A) Calcium Cytosol', 'c (uM)'),
        (0, 1, state_std[Cam, idx], state_p3[Cam, idx], '(B) Calcium Mito', 'Cam (uM)'),
        (1, 0, state_std[NADHm, idx], state_p3[NADHm, idx], '(C) NADH', 'NADHm (mM)'),
        (1, 1, state_std[PSIm, idx], state_p3[PSIm, idx], '(D) Potentiel Membrane', 'Psi (mV)'),
        (2, 0, algebrica_std[Jo, idx], algebrica_p3[Jo, idx], '(E) Respiration', 'Jo (uM/ms)'),
        (2, 1, 15.0 - state_std[ADPm, idx], 15.0 - state_p3[ADPm, idx], '(F) ATP', 'ATPm (mM)')
    ]

    for r, c_idx, d_std, d_p3, title, ylabel in plots:
        ax = axes[r, c_idx]
        ax.plot(t_min, d_std, 'k-', label='Standard')
        ax.plot(t_min, d_p3, 'k--', label='Sans activation p3')
        ax.set_title(title)
        ax.set_ylabel(ylabel)
        ax.grid(True, linestyle=':')
        if r == 2 and c_idx == 1: ax.legend()
